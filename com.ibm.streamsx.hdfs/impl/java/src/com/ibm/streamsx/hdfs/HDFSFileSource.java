...
public class HDFSFileSource extends AbstractHdfsOperator {
	...
	public synchronized void initialize(OperatorContext context)
		...
		if (file != null) {
			try {
				...
				logger.log(TraceLevel.DEBUG, "Unable to construct URI: " + e.getMessage());
	...
	public static void validateParameters(OperatorContextChecker checker)
		...
		/*
		 * If there is no input port, then,
		 * the parameter file would become mandatory.
		 * Set context as invalid otherwise
		 */
		if(streamingInputs.size() == 0 && !checker.getOperatorContext().getParameterNames().contains("file")){
			checker.setInvalidContext("HDFSFileSource requires either file parameter to be specified or the number of input ports to be one.", null);
		}
		
		/*
		 * If both input port and file parameter is specified,
		 * throw an exception
		 */
		if(streamingInputs.size() == 1 && checker.getOperatorContext().getParameterNames().contains("file")){
			checker.setInvalidContext("HDFSFileSource requires either file parameter to be specified or the number of input ports to be one. Both cannot be specified together.", null);
		}
	}
		...
		if (context.getParameterNames().contains("initDelay")){
			if(Integer.valueOf(context.getParameterValues("initDelay").get(0))<0){
				checker.setInvalidContext(
						...
						new String[] { context
								.getParameterValues("initDelay").get(0)
								.trim() });
			}
		}
	}
	...
	public static void checkOutputPortSchema(OperatorContextChecker checker)
		...
		if (outputSchema.getAttributeCount() != 1) {
			checker.setInvalidContext(
					"Output port can have only one attribute. The type of this attribute has to be rstring or ustring.", null);
		}
		
		if (outputSchema.getAttribute(0).getType()
				.getMetaType() != MetaType.RSTRING && outputSchema.getAttribute(0).getType()
				.getMetaType() != MetaType.USTRING) {
			checker.setInvalidContext(
					"Expected attribute of type rstring or ustring, found attribute of type "
	...
	public static void checkInputPortSchema(OperatorContextChecker checker)
		...
		if (streamingInputs.size() == 1) {
			...
			if (inputSchema.getAttributeCount() != 1) {
				checker.setInvalidContext(
						"Expect only one attribute on input port--attribute should be rstring giving a filename to open", null);
			}
	...
	public static void checkUriMatch(OperatorContextChecker checker) throws Exception {
		...
		if(hdfsUriValue != null && fileValue != null) {
			...
			try {
				...
				logger.log(TraceLevel.ERROR, "'hdfsUri' parameter contains an invalid URI: " + hdfsUriValue);
				throw e;
			}
			...
			try {
				...
				logger.log(TraceLevel.ERROR, "'file' parameter contains an invalid URI: " + fileValue);
				throw e;
			}
	...
	private void processFile(String filename) throws Exception {
		...
		BufferedReader reader = new BufferedReader(new InputStreamReader(
				dataStream, encoding), 1024*1024*8);
		...
		try {
			...
			logger.log(LogLevel.ERROR, "Exception occured during read: " + e.getMessage());
		} finally {
			dataStream.close();
			reader.close();
		}
	...
	public void process(StreamingInput<Tuple> stream, Tuple tuple)
		...
		//check if file name is an empty string. If so, log a warning and continue with the next tuple
		if(filename.isEmpty()){
...
//				hdfsClient.getInputStream(filename);
		...
		// TODO:  Process file already submits a marker, why do we need to do this here?
