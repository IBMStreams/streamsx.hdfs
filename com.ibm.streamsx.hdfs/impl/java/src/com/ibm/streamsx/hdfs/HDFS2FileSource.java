...
@PrimitiveOperator(name = "HDFS2FileSource", namespace = "com.ibm.streamsx.hdfs", description = IHdfsConstants.DESC_HDFS_FILE_SOURCE)

@Icons(location32 = "impl/java/icons/HDFS2FileSource_32.gif", location16 = "impl/java/icons/HDFS2FileSource_16.gif")

@InputPorts({
		@InputPortSet(description = IHdfsConstants.DESC_HDFS_FILE_SOURCE_INPUT, cardinality = 1, optional = true, controlPort = false, windowingMode = WindowMode.NonWindowed, windowPunctuationInputMode = WindowPunctuationInputMode.Oblivious) })

@OutputPorts({
		@OutputPortSet(description = IHdfsConstants.DESC_HDFS_FILE_SOURCE_OUTPUT, cardinality = 1, optional = false, windowPunctuationOutputMode = WindowPunctuationOutputMode.Generating) })

@SharedLoader
public class HDFS2FileSource extends AbstractHdfsOperator implements StateHandler {
	...
	public synchronized void initialize(OperatorContext context) throws Exception {
		if (fFileName != null) {
			try {
				...
				if (scheme != null) {
					...
					LOGGER.log(TraceLevel.DEBUG, "fileSystemUri: " + getHdfsUri());
				...
				LOGGER.log(TraceLevel.DEBUG, Messages.getString("HDFS_SOURCE_INVALID_URL", e.getMessage()));
		...
		TRACE.log(TraceLevel.INFO, "HDFS2FileSource - Data Governance - file: " + fFileName + " and HdfsUri: "
				+ getHdfsUri());
		...
		// logger.
		setLoggerAspects(LOGGER.getName(), "HDFS2FileSource");
		...
		if (MetaType.BLOB == outType) {
			fBinaryFile = true;
			TRACE.info("File will be read as a binary blobs of size " + fBlockSize);
		} else {
	...
	private void registerForDataGovernance(String serverURL, String file) {
		TRACE.log(TraceLevel.INFO, "HDFS2FileSource - Registering for data governance with server URL: " + serverURL
				+ " and file: " + file);
	...
	public static void validateParameters(OperatorContextChecker checker) throws Exception {
		...
		if (streamingInputs.size() == 0 && !checker.getOperatorContext().getParameterNames().contains("file")) {
			checker.setInvalidContext(Messages.getString("HDFS_SOURCE_INVALID_PARAM"), null);
		}

		/* If both input port and file parameter is specified, throw an
		 * exception */
		if (streamingInputs.size() == 1 && checker.getOperatorContext().getParameterNames().contains("file")) {
			checker.setInvalidContext(Messages.getString("HDFS_SOURCE_ONLY_ONE_PARAM"), null);
		}
	}
	...
	public static void validateParametersRuntime(OperatorContextChecker checker) throws Exception {
		...
		if (context.getParameterNames().contains("initDelay")) {
			if (Integer.valueOf(context.getParameterValues("initDelay").get(0)) < 0) {
				checker.setInvalidContext(Messages.getString("HDFS_SOURCE_INVALID_INIT_DELAY_PARAM"), new String[] {
						context.getParameterValues("initDelay").get(0).trim() });
			}
		}
	}
	...
	public static void checkOutputPortSchema(OperatorContextChecker checker) throws Exception {
		...
		if (outputSchema.getAttributeCount() != 1) {
			checker.setInvalidContext(Messages.getString("HDFS_SOURCE_INVALID_OUTPUT"), null);
		}

		if (outputSchema.getAttribute(0).getType().getMetaType() != MetaType.RSTRING && outputSchema.getAttribute(0)
				.getType().getMetaType() != MetaType.USTRING && outputSchema.getAttribute(0).getType()
						.getMetaType() != MetaType.BLOB) {
			checker.setInvalidContext(Messages.getString("HDFS_SOURCE_INVALID_ATTR_TYPE", outputSchema.getAttribute(0)
	...
	public static void checkInputPortSchema(OperatorContextChecker checker) throws Exception {
		...
		if (streamingInputs.size() == 1) {
			...
			if (inputSchema.getAttributeCount() != 1) {
				checker.setInvalidContext(Messages.getString("HDFS_SOURCE_INVALID_FILENAME_ATTR"), null);
			}

			// check that the attribute type must be a rstring
			if (MetaType.RSTRING != inputSchema.getAttribute(0).getType().getMetaType()) {
				checker.setInvalidContext(Messages.getString("HDFS_SOURCE_INVALID_STRING_ATTR", inputSchema
						.getAttribute(0).getType().getMetaType()), null);
			}
		}
	}
	...
	public static void checkUriMatch(OperatorContextChecker checker) throws Exception {
		...
		if (hdfsUriValue != null && fileValue != null) {
			...
			try {
				...
				LOGGER.log(TraceLevel.ERROR, Messages.getString("HDFS_SOURCE_INVALID_HDFS_URL", hdfsUriValue));
				throw e;
			}
			...
			try {
				...
				LOGGER.log(TraceLevel.ERROR, Messages.getString("HDFS_SOURCE_INVALID_FILE_URL", fileValue));
				throw e;
			}
			...
			if (fileUri.getScheme() != null) {
				...
				if (!hdfsUri.getScheme().equals(fileUri.getScheme())) {
					checker.setInvalidContext(Messages.getString("HDFS_SOURCE_INVALID_SCHEMA", fileUri.getScheme(),
				...
				if ((hdfsUri.getAuthority() == null && fileUri.getAuthority() != null) || (hdfsUri
						.getAuthority() != null && fileUri.getAuthority() == null) || (hdfsUri.getAuthority() != null
								&& fileUri.getAuthority() != null && !hdfsUri.getAuthority().equals(fileUri
										.getAuthority()))) {
					checker.setInvalidContext(Messages.getString("HDFS_SOURCE_INVALID_HOST", fileUri.getAuthority(),
	...
	private void initMetrics(OperatorContext context) {
		nOpenedFiles = context.getMetrics().createCustomMetric(FILES_OPENED_METRIC, "Number of opened files ",
				Metric.Kind.COUNTER);
	}
	...
	private void processFile(String filename) throws Exception {
		...
		nOpenedFiles.incrementValue(1);
		System.out.println(filename + " " + nOpenedFiles.getValue() + " files opened.");
		...
		try {
			...
			LOGGER.log(LogLevel.ERROR, Messages.getString("HDFS_SOURCE_EXCEPTION_READ_FILE"), e.getMessage());
		} finally {
			closeFile();
		}
	...
	private InputStream openFile(IHdfsClient hdfsClient, String filename) throws IOException {
	...
	private void doReadTextFile(InputStream dataStream, StreamingOutput<OutputTuple> outputPort, String filename)
			throws UnsupportedEncodingException, IOException, Exception {

		BufferedReader reader = new BufferedReader(new InputStreamReader(dataStream, fEncoding), BUFFER_SIZE);
		...
		do {
			try {
				...
				if (fSeekToLine >= 0) {
					...
					reader = new BufferedReader(new InputStreamReader(dataStream, fEncoding), BUFFER_SIZE);
	...
	private void doReadBinaryFile(InputStream dataStream, StreamingOutput<OutputTuple> outputPort) throws IOException,
			Exception {
	...
	public void reset(Checkpoint checkpoint) throws Exception {
		...
		if (!isDynamicFile()) {
			...
			if (fProcessThreadDone) {
				...
				// it inside the process method and end up having multiple
				// threads started
				startProcessing();
			}
		}
