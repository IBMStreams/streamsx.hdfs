...
public abstract class ByteRangeInputStream extends FSInputStream {
	...
	protected InputStream openInputStream() throws IOException {
		...
		if (isChunkedTransferEncoding(headers) || connection.getHeaderField(HttpHeaders.CONTENT_LENGTH) == null) {
	...
	private static long getLengthFromRange(String cl) throws IOException {
		try {
			...
			throw new IOException("failed to get content length by parsing the content range: " + cl + " " + e
					.getMessage());
		}
	}

	private static boolean isChunkedTransferEncoding(final Map<String, List<String>> headers) {
		return contains(headers, HttpHeaders.TRANSFER_ENCODING, "chunked") || contains(headers, HttpHeaders.TE,
				"chunked");
	}
