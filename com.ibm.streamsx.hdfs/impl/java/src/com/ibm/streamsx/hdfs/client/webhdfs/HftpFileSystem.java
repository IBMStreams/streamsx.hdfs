...
public class HftpFileSystem extends FileSystem implements DelegationTokenRenewer.Renewable,
	...
	protected static final ThreadLocal<SimpleDateFormat> df = new ThreadLocal<SimpleDateFormat>() {
		@Override
	...
	/**
	 * Initialize connectionFactory and tokenAspect. This function is intended
	 * to
	...
	public synchronized <T extends TokenIdentifier> void setDelegationToken(Token<T> token) {
		 ...
		 * We
		 * use the token for renewal, since the reflection utilities needs the
		 * value
		 * of the kind field to correctly renew the token.
		 * For other operations, however, the client has to send a
		 * HDFS_DELEGATION_KIND token over the wire so that it can talk to
		 * Hadoop
	...
	public synchronized Token<?> getDelegationToken(final String renewer) throws IOException {
		try {
			...
			final String proxyUser = connectUgi == null ? null : ugi.getShortUserName();
			if (connectUgi == null) {
				connectUgi = ugi;
			}
			return connectUgi.doAs(new PrivilegedExceptionAction<Token<?>>() {
				...
				public Token<?> run() throws IOException {
					Credentials c;
					try {
						c = DelegationTokenfetcher.getDTfromRemote(connectionFactory, nnUri, renewer, proxyUser);
					} catch (IOException e) {
						if (e.getCause() instanceof ConnectException) {
							LOG.warn("Couldn't connect to " + nnUri + ", assuming security is disabled");
							return null;
						}
	...
	protected URL getNamenodeURL(String path, String query) throws IOException {
		final URL url = new URL(getUnderlyingProtocol(), nnUri.getHost(), nnUri.getPort(), path + '?' + query);
	...
	private String getEncodedUgiParameter() {
		StringBuilder ugiParameter = new StringBuilder(JsonUtil.encodeQueryValue(ugi.getShortUserName()));
		for (String g : ugi.getGroupNames()) {
			ugiParameter.append(",");
			ugiParameter.append(JsonUtil.encodeQueryValue(g));
		}
		...
		if (UserGroupInformation.isSecurityEnabled()) {
			synchronized (this) {
				...
				if (delegationToken != null) {
					...
					return (query + JsonUtil.getDelegationTokenUrlParam(tokenString));
				}
			}
		}
		return query;
	...
	static class RangeHeaderInputStream extends ByteRangeInputStream {
		...
		RangeHeaderInputStream(URLConnectionFactory connFactory, final URL url) throws IOException {
	...
	public FSDataInputStream open(Path f, int buffersize) throws IOException {
		...
		String path = "/data" + JsonUtil.encodePath(f.toUri().getPath());
	...
	class LsParser extends DefaultHandler {
		...
		public void startElement(String ns, String localname, String qname, Attributes attrs) throws SAXException {
			...
			FileStatus fs = "file".equals(qname) ? new FileStatus(Long.parseLong(attrs.getValue("size")), false, Short
					.valueOf(attrs.getValue("replication")).shortValue(), Long.parseLong(attrs.getValue("blocksize")),
					modif, atime, FsPermission.valueOf(attrs.getValue("permission")), attrs.getValue("owner"), attrs
							.getValue("group"), HftpFileSystem.this.makeQualified(new Path(getUri().toString(), attrs
									.getValue("path"))))
					: new FileStatus(0L, true, 0, 0L, modif, atime, FsPermission.valueOf(attrs.getValue("permission")),
							attrs.getValue("owner"), attrs.getValue("group"), HftpFileSystem.this.makeQualified(
		...
		private void fetchList(String path, boolean recur) throws IOException {
			try {
				...
				HttpURLConnection connection = openConnection("/listPaths" + JsonUtil.encodePath(path), "ugi="
						+ getEncodedUgiParameter() + (recur ? "&recursive=yes" : ""));
	...
	private class ChecksumParser extends DefaultHandler {
		...
		public void startElement(String ns, String localname, String qname, Attributes attrs) throws SAXException {
			...
			// filechecksum =
			// MD5MD5CRC32FileChecksum.getCrcTypeFromAlgorithmName(localname);
			// filechecksum = MD5MD5CRC32FileChecksum.valueOf(attrs);
			// aannoo
		}

		private FileChecksum getFileChecksum(String f) throws IOException {
			final HttpURLConnection connection = openConnection("/fileChecksum" + JsonUtil.encodePath(f), "ugi="
					+ getEncodedUgiParameter());
			try {
	...
	private class ContentSummaryParser extends DefaultHandler {
		...
		private ContentSummary getContentSummary(String path) throws IOException {
			final HttpURLConnection connection = openConnection("/contentSummary" + JsonUtil.encodePath(path), "ugi="
					+ getEncodedUgiParameter());
			InputStream in = null;
			try {
	...
	private static ContentSummary toContentSummary(Attributes attrs) throws SAXException {
		...
		if (length == null || fileCount == null || directoryCount == null || quota == null || spaceConsumed == null
				|| spaceQuota == null) {
			return null;
		}

		try {
			return new ContentSummary(Long.parseLong(length), Long.parseLong(fileCount), Long.parseLong(directoryCount),
					Long.parseLong(quota), Long.parseLong(spaceConsumed), Long.parseLong(spaceQuota));
		} catch (Exception e) {
			throw new SAXException("Invalid attributes: length=" + length + ", fileCount=" + fileCount
		...
		try {
			return connectUgi.doAs(new PrivilegedExceptionAction<Long>() {
				...
				public Long run() throws Exception {
					...
					return DelegationTokenfetcher.renewDelegationToken(connectionFactory, DFSUtil.createUri(
							getUnderlyingProtocol(), serviceAddr), (Token<DelegationTokenIdentifier>) token);
				}
			});
		} catch (InterruptedException e) {
		...
		try {
			connectUgi.doAs(new PrivilegedExceptionAction<Void>() {
				...
				public Void run() throws Exception {
					InetSocketAddress serviceAddr = SecurityUtil.getTokenServiceAddr(token);
					DelegationTokenfetcher.cancelDelegationToken(connectionFactory, DFSUtil.createUri(
							getUnderlyingProtocol(), serviceAddr), (Token<DelegationTokenIdentifier>) token);
					return null;
				}
			});
