 ...
 * It is modified to support HttpURLConnection and WebHdfsFileSystem
 */
...
public class WebHdfsFileSystem extends FileSystem implements DelegationTokenRenewer.Renewable,
	...
	public synchronized void initialize(URI uri, Configuration conf) throws IOException {
		...
		this.tokenServiceName = isLogicalUri ? HAUtilClient.buildTokenServiceForLogicalUri(uri, getScheme())
		...
		if (!isHA) {
			...
			int maxFailoverAttempts = conf.getInt(HdfsClientConfigKeys.HttpClient.FAILOVER_MAX_ATTEMPTS_KEY,
					HdfsClientConfigKeys.HttpClient.FAILOVER_MAX_ATTEMPTS_DEFAULT);
			int maxRetryAttempts = conf.getInt(HdfsClientConfigKeys.HttpClient.RETRY_MAX_ATTEMPTS_KEY,
					HdfsClientConfigKeys.HttpClient.RETRY_MAX_ATTEMPTS_DEFAULT);
			int failoverSleepBaseMillis = conf.getInt(HdfsClientConfigKeys.HttpClient.FAILOVER_SLEEPTIME_BASE_KEY,
					HdfsClientConfigKeys.HttpClient.FAILOVER_SLEEPTIME_BASE_DEFAULT);
			int failoverSleepMaxMillis = conf.getInt(HdfsClientConfigKeys.HttpClient.FAILOVER_SLEEPTIME_MAX_KEY,
	...
	public static boolean isEnabled(final Configuration conf, final Log log) {
		...
		final boolean b = conf.getBoolean(DFS_WEBHDFS_ENABLED_KEY, DFS_WEBHDFS_ENABLED_DEFAULT);
		return b;
	}

	TokenSelector<DelegationTokenIdentifier> tokenSelector = new AbstractDelegationTokenSelector<DelegationTokenIdentifier>(
			getTokenKind()) {
	};
	...
	private static Map<?, ?> validateResponse(final HttpOpParam.Op op, final HttpURLConnection conn,
		...
		if (code != op.getExpectedHttpResponseCode()) {
			...
			try {
				...
				throw new IOException("Unexpected HTTP response: code=" + code + " != " + op
						.getExpectedHttpResponseCode() + ", " + op.toQueryString() + ", message=" + conn
	...
	protected Param<?, ?>[] getAuthParameters(final HttpOpParam.Op op) throws IOException {
	...
	protected URL toUrl(final HttpOpParam.Op op, final Path fspath, final Param<?, ?>... parameters)
			throws IOException {
		// initialize URI path and query
		final String path = getPathPrefix() + (fspath == null ? "/" : makeQualified(fspath).toUri().getRawPath());
		final String query = op.toQueryString() + Param.toSortedString("&", getAuthParameters(op)) + Param
				.toSortedString("&", parameters);
	...
	abstract class AbstractRunner<T> {
		 ...
		 * 100-continue". The two-step create/append is a temporary workaround
		 * for
		...
		private HttpURLConnection connect(URL url) throws IOException {
			...
			if (op.getRedirect() && !redirected) {
				...
				// application level proxy like httpfs might not issue a
				// redirect
		...
		private HttpURLConnection connect(final HttpOpParam.Op op, final URL url) throws IOException {
			...
			case PUT: {
				...
				if (!doOutput) {
					...
					// opening and closing the stream will send "Content-Length:
					// 0"
		...
		private T runWithRetry() throws IOException {
			 ...
			 *      <li>The namenode enters the standby state (i.e.,
			 *      StandbyException).</li>
			 *      <li>The server returns errors for the command (i.e.,
			 *      RemoteException)</li>
			 *      </ul>
			 *      The call to shouldRetry() will conduct the retry policy. The
			 *      policy
			 *      examines the exception and swallows it if it decides to
			 *      rerun the work.
			 */
			for (int retry = 0;; retry++) {
				...
				try {
					...
					// try to replace the expired token with a new one. the
					// attempt
					// to acquire a new token must be outside this operation's
					// retry
					// so if it fails after its own retries, this operation
					// fails too.
			...
			if (checkRetry) {
				try {
					...
					if (isRetry || isFailoverAndRetry) {
						LOG.info("Retrying connect to namenode: " + nnAddr + ". Already tried " + retry
								+ " time(s); retry policy is " + retryPolicy + ", delay " + a.delayMillis + "ms.");
	...
	abstract class AbstractFsPathRunner<T> extends AbstractRunner<T> {
		...
		AbstractFsPathRunner(final HttpOpParam.Op op, final Path fspath, Param<?, ?>... parameters) {
			super(op, false);
			this.fspath = fspath;
			this.parameters = parameters;
		}

		AbstractFsPathRunner(final HttpOpParam.Op op, Param<?, ?>[] parameters, final Path fspath) {
			super(op, false);
			this.fspath = fspath;
			this.parameters = parameters;
		}
	...
	abstract class FsPathResponseRunner<T> extends AbstractFsPathRunner<T> {
		FsPathResponseRunner(final HttpOpParam.Op op, final Path fspath, Param<?, ?>... parameters) {
			super(op, fspath, parameters);
		}

		FsPathResponseRunner(final HttpOpParam.Op op, Param<?, ?>[] parameters, final Path fspath) {
	...
	class FsPathOutputStreamRunner extends AbstractFsPathRunner<FSDataOutputStream> {
		...
		FsPathOutputStreamRunner(Op op, Path fspath, int bufferSize, Param<?, ?>... parameters) {
			super(op, fspath, parameters);
	...
	private HdfsFileStatus getHdfsFileStatus(Path f) throws IOException {
		// System.out.println("getHdfsFileStatus " + " " + f);
	...
	public FileStatus getFileStatus(Path f) throws IOException {
		// System.out.println("getFileStatus " + " " + f);
	...
	private FileStatus makeQualified(HdfsFileStatus f, Path parent) {
		...
		return new FileStatus(f.getLen(), f.isDir(), f.getReplication(), f.getBlockSize(), f.getModificationTime(), f
				.getAccessTime(), f.getPermission(), f.getOwner(), f.getGroup(), symLinkPath,
...
//	@SuppressWarnings("deprecation")
	...
	public void rename(final Path src, final Path dst, final Options.Rename... options) throws IOException {
		statistics.incrementWriteOps(1);
		final HttpOpParam.Op op = PutOpParam.Op.RENAME;
		new FsPathRunner(op, src, new DestinationParam(makeQualified(dst).toUri().getPath()), new RenameOptionSetParam(
				options)).run();
	}
	...
	public void setXAttr(Path p, String name, byte[] value, EnumSet<XAttrSetFlag> flag) throws IOException {
		...
		if (value != null) {
			new FsPathRunner(op, p, new XAttrNameParam(name), new XAttrValueParam(XAttrCodec.encodeValue(value,
					XAttrCodec.HEX)), new XAttrSetFlagParam(flag)).run();
		} else {
	...
	public byte[] getXAttr(Path p, final String name) throws IOException {
		...
		return new FsPathResponseRunner<byte[]>(op, p, new XAttrNameParam(name), new XAttrEncodingParam(
				XAttrCodec.HEX)) {
			@Override
	...
	public void renameSnapshot(final Path path, final String snapshotOldName, final String snapshotNewName)
		...
		new FsPathRunner(op, path, new OldSnapshotNameParam(snapshotOldName), new SnapshotNameParam(snapshotNewName))
				.run();
	}
	...
	public FSDataOutputStream create(final Path f, final FsPermission permission, final boolean overwrite,
		...
		return new FsPathOutputStreamRunner(op, f, bufferSize, new PermissionParam(applyUMask(permission)),
				new OverwriteParam(overwrite), new BufferSizeParam(bufferSize), new ReplicationParam(replication),
	...
	public FSDataInputStream open(final Path f, final int buffersize) throws IOException {
		...
		return new FSDataInputStream(new OffsetUrlInputStream(new UnresolvedUrlOpener(runner), new OffsetUrlOpener(
				null)));
	}
	...
	public Token<DelegationTokenIdentifier> getDelegationToken(final String renewer) throws IOException {
		...
		Token<DelegationTokenIdentifier> token = new FsPathResponseRunner<Token<DelegationTokenIdentifier>>(op, null,
	...
	@Override
	public BlockLocation[] getFileBlockLocations(final FileStatus status, final long offset, final long length)
			throws IOException {
		if (status == null) {
			return null;
		}
	...
	@Override
	public BlockLocation[] getFileBlockLocations(final Path p, final long offset, final long length)
			throws IOException {
		statistics.incrementReadOps(1);
		...
		return new FsPathResponseRunner<BlockLocation[]>(op, p, new OffsetParam(offset), new LengthParam(length)) {
			...
			BlockLocation[] decodeResponse(Map<?, ?> json) throws IOException {
				...
				if (isWebHDFSJson(json)) {
					return DFSUtilClient.locatedBlocks2Locations(JsonUtil.toLocatedBlocks(json));
				} else {
					return JsonUtil.toBlockLocations(json);
				}
			}
		}.run();
	}
	...
	/**
	 * Check whether the Json comes from WebHDFS
	 * 
	 * @param json
	 * @return
	 */
	private boolean isWebHDFSJson(Map<?, ?> json) {
		Map<?, ?> m = (Map<?, ?>) json.get("LocatedBlocks");
		return m != null && m.containsKey("isLastBlockComplete");
	}
	 ...
	 * logical name(i.e., the authority in the URL) into real namenode
	 * addresses.
	 */
